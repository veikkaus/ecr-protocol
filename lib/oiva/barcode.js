...
function drawGameTicketSerialToITF(ticketSerial) {
  return ticketSerial
    .trim()
    .replace(/[^0-9]/g, '')
    .padStart(20, '0')
...
function ticketSerialToBarcodeData(ticketSerial, typeOfGame) {
  switch (typeOfGame) {
    ...
    case gameType.KaikkiTaiEiMitaan:
      return drawGameTicketSerialToITF(ticketSerial)
  }
}

function createImage(ticketSerial, itfValue) {
  ...
  try {
    const filePath = path.resolve(`${Date.now()}-${ticketSerial}.png`)
...
function createBarcodeImage(ticketSerial, typeOfGame, imageCreator = createImage) {
  imageCreator(ticketSerial, ticketSerialToBarcodeData(ticketSerial, typeOfGame))
}

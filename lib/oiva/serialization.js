...
function serializedMessage(message, useRandomTicketId) {
  ...
  return serializedOivaMessageFromComponents(
    ...
    useRandomTicketId ? generateRandomTicketId(message.gameType) : message.ticketId,
    message.shareTicketIds?.length?.toString() ?? '0',
    ...(message.shareTicketIds?.map(ticketId => useRandomTicketId ? generateRandomTicketId(message.gameType) : ticketId) ?? [])
  )
}

...
describe('Oiva serialization', () => {
  test.each([
    ...
    expect(serializedMessage(messageObject, false)).toEqual(expectedSerialization)
  })
  ...
  it('should randomize message ticket ids', () => {
    const randomTicketIdsInSerializedMessage = (messageObject, ticketIdRegex) => {
      return serializedMessage(messageObject, true)
          .split(';')
    ...
    expect(randomTicketIdsInSerializedMessage(totoMessage, /^A-AAAA-BBBB-[0-9A-F]{4}$/).length).toEqual(3)
    ...
    expect(randomTicketIdsInSerializedMessage(lottoMessage, /^1234-111111111-08[0-9]{4}$/).length).toEqual(1)
  })
})
